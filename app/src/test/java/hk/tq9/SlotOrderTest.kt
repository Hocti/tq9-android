...
class SlotOrderTest {
    ...
    @Test fun `第一頁順住由 1 排到 9`() {
        assertEquals(natural, Q9Engine.slotOrder(0))
    }

    @Test fun `第二頁開始由中間格排起`() {
        for (page in 1..4) assertEquals(centre, Q9Engine.slotOrder(page))
    }

    /** 九個格一個唔少一個唔多 */
    @Test fun `每頁都係 1 到 9 九個格`() {
        for (page in 0..4) assertEquals(natural, Q9Engine.slotOrder(page).sorted())
    }
}
