...
/**
 * 「常用字排前」點排（`Q9Engine.reorderByUsage`）。
 *
...
class UsageReorderTest {
    ...
    private fun sort(words: List<String>, counts: Map<String, Int> = emptyMap()) =
        Q9Engine.reorderByUsage(words) { counts[it] ?: 0 }
    ...
    @Test fun `常用但唔喺頭九位嘅字推到第十位`() {
        ...
        assertEquals(
            head + listOf("百", "十", "廿", "卅"),
            sort(words, mapOf("百" to Q9Engine.MIN_USAGE_COUNT))
        )
    }
    ...
    @Test fun `未打夠 MIN_USAGE_COUNT 次唔算常用`() {
        ...
        val counts = mapOf("卅" to Q9Engine.MIN_USAGE_COUNT - 1)
    ...
    @Test fun `佔位符同非中文唔會被推前`() {
        val words = head + listOf(Q9Engine.PLACEHOLDER, "，", "A", "百")
        val counts = mapOf(Q9Engine.PLACEHOLDER to 99, "，" to 99, "A" to 99, "百" to 2)
        assertEquals(
            head + listOf("百", Q9Engine.PLACEHOLDER, "，", "A"),
            sort(words, counts)
        )
    }
