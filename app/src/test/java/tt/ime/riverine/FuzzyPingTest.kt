...
/**
 * 近音字靠 [TTDb.fuzzyPing]：兩個 `ping` 揉到同一個結果就當近音
 * （`nearHomo` 就係攞呢個做 key 分組）。
 */
class FuzzyPingTest {

    private fun same(a: String, b: String) =
        assertEquals("$a 同 $b 應該當近音", TTDb.fuzzyPing(a), TTDb.fuzzyPing(b))

    private fun diff(a: String, b: String) =
        assertNotEquals("$a 同 $b 唔應該撞埋", TTDb.fuzzyPing(a), TTDb.fuzzyPing(b))
    ...
    @Test fun `單鼻音字唔會揉到剩返吉`() {
        for (p in listOf("ng", "m", "hng")) {
            assertNotEquals("「$p」揉完變吉", "", TTDb.fuzzyPing(p))
        }
    }
}
