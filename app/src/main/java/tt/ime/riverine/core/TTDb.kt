...
/**
 * 字碼資料庫。對應 Windows 版嘅核心資料層。
 *
 * 資料庫檔案放喺 filesDir/dataset.db，第一次開機由 assets 複製過去，
 * **裝咗新版 apk 亦都會抄多次**（見 [TTDb.ensureInstalled]）。
 * setting page 可以揀另一個 sqlite file 直接覆蓋（舊版唔會保留），
 * 換過之後就當係 user 自己嘅嘢，升級唔會再踩親。
 */
class TTDb private constructor(private val db: SQLiteDatabase) {
    ...
    /**
     * 打咗一兩個碼、仲未夠碼出關聯字嗰陣，上面條 bar 出「呢個碼開頭最常用嗰幾隻字」。
     *
    ...
    companion object {
        ...
        private fun validate(f: File) {
            ...
            d.use {
                ...
                require(missing.isEmpty()) { "不是有效的三三資料庫，缺少：" + missing.joinToString(", ") }
            }
        }

        fun open(ctx: Context): TTDb {
            ensureInstalled(ctx)
            val d = SQLiteDatabase.openDatabase(
                file(ctx).absolutePath, null,
                SQLiteDatabase.OPEN_READONLY or SQLiteDatabase.NO_LOCALIZED_COLLATORS
            )
            return TTDb(d).also { it.loadWeights() }
        }
